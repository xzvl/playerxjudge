...
export default async function FinalStagePage({ params }: { params: Promise<{ slug: string }> }) {
  const { slug } = await params;
  const user = await getCurrentUser();
  if (!user) redirect(`/login?redirectTo=/account/organizer/tournament/${slug}/final-stage`);
  ...
  if (tournament.format_settings?.stageType !== "two_stage") {
    ...
    redirect(`/account/organizer/tournament/${slug}`);
  }
  ...
  if (!isSwiss) {
    ...
    // preview — only Swiss has real group-stage data to seed from so far.
